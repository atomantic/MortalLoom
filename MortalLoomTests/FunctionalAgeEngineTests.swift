...
final class FunctionalAgeEngineTests: XCTestCase {
    ...
    func testExpectedWalkingSpeedPlateausThroughMidlife() {
        // Clinical plateau (1.40) minus the free-living offset (0.25) → 1.15 m/s,
        // flat at and below the anchor age.
        XCTAssertEqual(FunctionalAgeEngine.expectedWalkingSpeed(age: 30), 1.15, accuracy: 0.0001)
        XCTAssertEqual(FunctionalAgeEngine.expectedWalkingSpeed(age: 50), 1.15, accuracy: 0.0001)
    }

    func testExpectedWalkingSpeedDeclinesAfterAnchor() {
        // 0.012 m/s per year past 50: at 70 → (1.40 − 20·0.012) − 0.25 = 0.91.
        XCTAssertEqual(FunctionalAgeEngine.expectedWalkingSpeed(age: 70), 0.91, accuracy: 0.0001)
    }

    func testExpectedStairSpeedPlateauAndDecline() {
        // Clinical stair anchor (0.50) minus the free-living stair offset (0.05).
        XCTAssertEqual(FunctionalAgeEngine.expectedStairSpeed(age: 30), 0.45, accuracy: 0.0001)
        XCTAssertEqual(FunctionalAgeEngine.expectedStairSpeed(age: 70), 0.35, accuracy: 0.0001)
    }

    // MARK: - Walking-speed inversion (age-relative)

    func testWalkingAtAgeNormalReadsOwnAge() {
        // Walking the everyday-equivalent speed expected at one's age reads as that age.
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromWalkingSpeed(1.15, chronologicalAge: 50), 50, accuracy: 0.0001)
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromWalkingSpeed(0.91, chronologicalAge: 70), 70, accuracy: 0.0001)
    }

    func testWalkingUnder50PlateauNotPenalized() {
        // Regression: a healthy 30-year-old at the everyday plateau speed must read ~30,
        // NOT be mapped to the anchor age (50) as if the plateau were a decline.
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromWalkingSpeed(1.15, chronologicalAge: 30), 30, accuracy: 0.0001)
    }

    func testFasterThanAgeNormalReadsYounger() {
        // +0.12 m/s above the age-50 everyday norm = 10 years younger (0.012 m/s per year).
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromWalkingSpeed(1.27, chronologicalAge: 50), 40, accuracy: 0.0001)
    }

    func testSlowerThanAgeNormalReadsOlder() {
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromWalkingSpeed(1.03, chronologicalAge: 50), 60, accuracy: 0.0001)
    }
    ...
    func testStairAtAgeNormalReadsOwnAge() {
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromStairSpeed(0.45, chronologicalAge: 50), 50, accuracy: 0.0001)
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromStairSpeed(0.45, chronologicalAge: 30), 30, accuracy: 0.0001)
    }

    func testStairSpeedSteeperSlope() {
        // 0.005 m/s per year: ±0.05 m/s = ±10 years vs the age-50 everyday norm (0.45).
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromStairSpeed(0.50, chronologicalAge: 50), 40, accuracy: 0.0001)
        XCTAssertEqual(FunctionalAgeEngine.functionalAgeFromStairSpeed(0.40, chronologicalAge: 50), 60, accuracy: 0.0001)
    }
    ...
    func testEstimateReturnsNilForUnknownAge() {
        ...
        XCTAssertNil(FunctionalAgeEngine.estimate(
            walkingSpeed: 1.15, stairSpeedUp: 0.45, stairSpeedDown: 0.45,
            asymmetry: nil, chronologicalAge: 0))
    }

    func testEstimateFromWalkingSpeedOnly() {
        // 0.12 m/s below the age-50 everyday norm (1.15) → 10 years older.
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: 1.03, stairSpeedUp: nil, stairSpeedDown: nil,
    ...
    func testEverydayNormalSpeedReadsOnPar() {
        // Regression for the free-living recalibration: a 46-year-old whose everyday
        // HealthKit walking average is 1.14 m/s (rated "Good" by GaitEngine) must read
        // ~on par — not +22 years older, which the pre-offset model produced.
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: 1.14, stairSpeedUp: nil, stairSpeedDown: nil,
            asymmetry: nil, chronologicalAge: 46)
        XCTAssertEqual(summary?.level, .onPar)
        XCTAssertLessThanOrEqual(abs(summary?.gapYears ?? .nan), 3)
    }

    func testEstimateUnder50AtPlateauReadsOnPar() {
        // Regression for the under-50 plateau: a healthy 30-year-old at age-normal
        // everyday mobility must read ~30 / On Par, not be penalized toward the anchor.
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: 1.15, stairSpeedUp: 0.45, stairSpeedDown: 0.45,
    ...
    func testEstimateAveragesStairUpAndDown() {
        // Stair up 0.50 (→40) and down 0.40 (→60) average to 0.45 (→50, on par).
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: nil, stairSpeedUp: 0.50, stairSpeedDown: 0.40,
            asymmetry: nil, chronologicalAge: 50)
        XCTAssertEqual(summary?.functionalAge ?? .nan, 50, accuracy: 0.0001)
        XCTAssertEqual(summary?.avgStairSpeed ?? .nan, 0.45, accuracy: 0.0001)
        XCTAssertEqual(summary?.componentCount, 1)
    }

    func testEstimateBlendsWalkingAndStairThenAdjustsForAsymmetry() {
        // Walking 1.27 → 40, stair 0.50 → 40; base 40. Asymmetry 7 adds +3 → 43.
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: 1.27, stairSpeedUp: 0.50, stairSpeedDown: 0.50,
        ...
        XCTAssertEqual(summary?.asymmetryAdjustment ?? .nan, 3, accuracy: 0.0001)
    }

    func testEstimatePopulatesComponentBreakdown() {
        // The breakdown surfaces each signal's measured/expected speed and implied gap
        // so the UI can explain the number.
        let summary = FunctionalAgeEngine.estimate(
            walkingSpeed: 1.03, stairSpeedUp: 0.45, stairSpeedDown: 0.45,
            asymmetry: nil, chronologicalAge: 50)
        XCTAssertEqual(summary?.components.count, 2)
        let walk = summary?.components.first { $0.kind == .walkingSpeed }
        XCTAssertEqual(walk?.measured ?? .nan, 1.03, accuracy: 0.0001)
        XCTAssertEqual(walk?.expected ?? .nan, 1.15, accuracy: 0.0001)
        XCTAssertEqual(walk?.gapYears ?? .nan, 10, accuracy: 0.0001)
        // Walking (gap +10) drives harder than stairs (gap 0).
        XCTAssertEqual(summary?.primaryDriver?.kind, .walkingSpeed)
    }
    ...
    func testSummarizeAveragesAcrossEntries() {
        let metrics = [
            HealthMetricEntry(date: "2026-01-01", walkingSpeed: 1.15, stairSpeedUp: 0.45, stairSpeedDown: 0.45),
            HealthMetricEntry(date: "2026-01-02", walkingSpeed: 1.15, stairSpeedUp: 0.45, stairSpeedDown: 0.45)
        ]
