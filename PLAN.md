...
- [x] **Weekly review sparkline.** `WeeklyReviewSheet.reviewStep` — 7-day inline `Chart` plotting daily `alignmentRating` values from the apex's reflection check-ins. Latest entry per calendar day wins, days without a rating are omitted from the data array (Charts renders them as gaps). Y-axis pinned 1–10 with marks at 1/5/10; X-axis shows narrow weekday labels (M T W T F S S) across the rolling 7-day domain. Hidden when no apex or no recent ratings exist.
